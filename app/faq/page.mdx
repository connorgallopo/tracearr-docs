...
- [Does Tracearr need a mail provider?](#does-tracearr-need-a-mail-provider)
- [Is a contact email the same as a login email?](#is-a-contact-email-the-same-as-a-login-email)
...
- [Jellyfin 10.11.9+: Playback Reporting check fails with a 500 error](#jellyfin-10119-playback-reporting-check-fails-with-a-500-error)
...
- [My newsletter says it sent but nobody got it](#my-newsletter-says-it-sent-but-nobody-got-it)
- [Members on my Jellyfin server get no links to the items](#members-on-my-jellyfin-server-get-no-links-to-the-items)
...
- [I set a server location but past local plays still show as Local Network](#i-set-a-server-location-but-past-local-plays-still-show-as-local-network)
- [My Tautulli history imported before Tracearr 1.3.9 shows no progress](#my-tautulli-history-imported-before-tracearr-139-shows-no-progress)
...
Yes. Tracearr imports from Tautulli (Plex), directly from the Playback Reporting plugin (Jellyfin and Emby), and from a Jellystat backup file. See the [import guide](/getting-started/import).

---

### Does Tracearr need a mail provider?

No. Tracearr speaks SMTP and nothing else, so any relay works, including one running on the same machine as Tracearr. The seven provider presets are there to save you looking up a host and a port; **Custom** takes whatever you have. See [Email & Newsletters](/configuration/email).

---

### Is a contact email the same as a login email?

No, they are two different fields on two different things. A Tracearr account's email is part of signing in and is what the [`set-email`](/configuration/recovery#set-email) recovery command changes. A contact email is an address the owner types for a person so newsletters have somewhere to go. It grants no login, and it is never used to match a synced media server account to a Tracearr account.
...
Yes, on Proxmox. The [community helper script](https://community-scripts.github.io/ProxmoxVE/scripts?id=tracearr), which we maintain, installs Tracearr, PostgreSQL with TimescaleDB, and Redis into an LXC container with no Docker involved. That is the only non-Docker install we support. See [Upgrading](/upgrading#proxmox-ve) for how to update it.

Anything else (bare metal, your own VM, a distro package) is on you. There is no written guide and we don't recommend it. The pieces are all in the repo if you want to work it out: [`docker/Dockerfile`](https://github.com/connorgallopo/Tracearr/blob/main/docker/Dockerfile) shows how the app is built with pnpm and started (`node apps/server/dist/index.js` on Node 22 or newer), [`docker/docker-compose.dev.yml`](https://github.com/connorgallopo/Tracearr/blob/main/docker/docker-compose.dev.yml) shows the services it expects next to it, and [`docker/Dockerfile.supervised`](https://github.com/connorgallopo/Tracearr/blob/main/docker/Dockerfile.supervised) installs all three on one Debian image, which is as close to a manual install as the repo gets. The Proxmox script is worth reading for the same reason, since it puts the same stack onto a plain OS without containers.

The database is where this gets hard. Tracearr needs PostgreSQL with the TimescaleDB and `timescaledb_toolkit` extensions loaded and `pg_trgm` available, plus Redis. A plain PostgreSQL install does not work: the migrations create hypertables, continuous aggregates, and compression and retention policies, and they fail on the first run without TimescaleDB. PostgreSQL also has to run with settings Tracearr depends on. Read them off the `command:` line in [`docker-compose.pg18.yml`](https://github.com/connorgallopo/Tracearr/blob/main/docker/examples/docker-compose.pg18.yml), in particular `max_locks_per_transaction=4096` and `max_connections=150`, along with 512MB of shared memory. Leave those at stock values and a large Tautulli import runs out of lock table space.
...
2. Check that your API key/token is valid. A replaced Jellyfin or Emby key goes in **Edit Server**, see [Changing the URL or API key](/configuration/servers#changing-the-url-or-api-key)
...
1. In Jellyfin, go to **Dashboard** → **Plugins**, open **Playback Reporting**, and uninstall it
2. Restart Jellyfin
...
Nothing on the Tracearr side can work around this; the exception is thrown inside the plugin before Tracearr's query is read. See [issue #1001](https://github.com/connorgallopo/Tracearr/issues/1001) for the full trace and the upstream Jellyfin and plugin changes.
...
2. Check that your API key or token is still valid. A replaced Jellyfin or Emby key goes in **Edit Server**, see [Changing the URL or API key](/configuration/servers#changing-the-url-or-api-key)
...
> **Note:** Local streams (those from your home network) have a private IP that no geolocation database can place, so they show as **Local Network** and stay off the map. To put them on the map, give the server a [location](/configuration/servers#server-location). If your _remote_ streams are showing as "local" or missing location data, continue reading.
...
docker exec -it tracearr node apps/server/dist/scripts/reset-password.js newpassword123
...
```bash
# Interactive mode (you will be prompted to enter a new password)
node /opt/tracearr/apps/server/dist/scripts/reset-password.js

# Non-interactive mode (sets password to "newpassword123")
node /opt/tracearr/apps/server/dist/scripts/reset-password.js newpassword123
````
...
- **Jellyfin and Emby** don't expose CPU or memory through their APIs at all. Installing the [SSE plugin](/sse-plugin) (v0.4 or newer) fills the panel with a live sample every 6 seconds. Host-level values require the media server to run on Linux, including in a container.
...
### I set a server location but past local plays still show as Local Network

Saving a [server location](/configuration/servers#server-location) runs **Apply Server Locations** on its own. If another job was running at the time, it starts when that one finishes. If past plays still show as Local Network after that, run **Apply Server Locations** in Settings → Jobs, which rechecks every server.

---

### My Tautulli history imported before Tracearr 1.3.9 shows no progress

Run **Fix Imported Session Progress** in Settings → Jobs. It estimates progress and length for plays that have neither, and never changes a play that already has either value.
