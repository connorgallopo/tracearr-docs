...
Treat these values like passwords. Don't share them, don't commit them to Git, and don't post them in support channels. If you think your secrets have been exposed, generate new ones and restart Tracearr — all existing sessions will be invalidated and users will need to log in again. Note that rotating `JWT_SECRET` also changes the derived `BETTER_AUTH_SECRET` when it is not set explicitly, which invalidates web sessions and paired mobile devices, not just legacy tokens.
</Callout>
