...
The Quick Start commands above handle this automatically, but if you need to generate them separately (for example, if you're deploying through [Portainer](/getting-started/installation/portainer) or another GUI), here are a few ways to do it.

### Using a terminal

This is the most straightforward method and produces cryptographically secure output.
...
Run the command twice — once for `JWT_SECRET` and once for `COOKIE_SECRET`. Copy each output and paste it into your `.env` file or environment variable configuration.

### Using an online generator

If you don't have terminal access (or just prefer a browser), you can use [randomkeygen.com/random-string](https://randomkeygen.com/random-string):
...
You'll notice the compose files use **named Docker volumes** (like `timescale_data` and `redis_data`) rather than bind mounts (like `./data:/var/lib/postgresql/data`). This is deliberate, and it matters more than you might expect.
...
The [supervised image](#about-the-supervised-image) makes this even more critical. It runs three separate processes inside a single container — PostgreSQL, Redis, and Tracearr — each as a different system user (`postgres`, `redis`, `tracearr`), each writing to its own data directory. With bind mounts, you'd need to manually ensure correct ownership for each one and keep those permissions intact across container updates, host reboots, and filesystem changes. Named volumes handle all of this automatically.
...
<Tabs items={['Standard (separate db container)', 'Supervised (all-in-one)']} defaultIndex={0}>
<Tabs.Tab>
```bash
...
docker exec -i tracearr-db psql -U tracearr -d tracearr < tracearr_backup.sql
```
</Tabs.Tab>
<Tabs.Tab>
```bash
# Back up the database (the db runs inside the tracearr container)
docker exec tracearr pg_dump -U tracearr -d tracearr -Fc > tracearr_backup.dump

# Back up to plain SQL
...
You can schedule this with a cron job, or run it manually whenever you want a snapshot. The `-Fc` (custom format) option compresses the output and supports selective restoration of individual tables if you ever need it.
...
docker exec -i tracearr-db pg_restore -U tracearr -d tracearr < tracearr_backup.dump
```

The TimescaleDB extension will be recreated by `pg_restore` from the dump file — no extra steps needed.
</Callout>
...
<Callout type="error" title="Don't mix PostgreSQL major versions">
The different compose files use different PostgreSQL versions — the supervised image runs PG15, the standard compose uses PG16, and the recommended compose uses PG18. PostgreSQL's on-disk binary format changes between major versions, so **you cannot swap to a different compose file and reuse the same data volume**. If you try, PostgreSQL will refuse to start. Always `pg_dump` your data before switching compose files.
</Callout>
