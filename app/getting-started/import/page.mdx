...
If the check says the plugin isn't installed and shows `500 Internal Server Error`, but Playback Reporting is installed and working, your Jellyfin has a plugin build that predates an API change in 10.11.9. Uninstall and reinstall the plugin from the catalog; the updater won't offer the fix on its own. See the [FAQ entry](/faq#jellyfin-10119-playback-reporting-check-fails-with-a-500-error) for why.
</Callout>
...
| **Enrich with media metadata** | Fetches season/episode numbers, year, artwork, and runtime from your server. *(Recommended)* |
