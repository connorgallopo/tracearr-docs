...
Two API paths answer without a login by design, and an authenticating proxy such as Authelia, Authentik or a Cloudflare Access policy must let them through:
...
The first is the unsubscribe page every newsletter links to; the second is the browser view of a digest that was sent. Gating either one shows members a login page instead of the page they asked for. Both carry their own limit of 60 requests a minute.
