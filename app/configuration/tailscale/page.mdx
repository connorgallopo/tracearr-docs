...
- <span id="outbound-connections">**Make outbound connections over the tailnet**</span> — Userspace networking only supports incoming connections. Tracearr cannot use its Tailscale interface to reach other devices on your tailnet. Tailscale's userspace mode does provide a SOCKS5 proxy that could enable outbound connections, but this is not currently implemented in Tracearr. <sup>1</sup>
