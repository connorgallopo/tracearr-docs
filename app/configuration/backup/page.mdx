...
<Callout type="warning" title="Beta Feature">
  Backup & Restore is currently in **beta**. It has been tested internally but needs broader testing
  with real-world data. Please report any issues you encounter.
</Callout>
...
<Callout type="error" title="Destructive Operation">
  Restoring a backup replaces your entire database. All current data will be overwritten. A rollback
  point is created automatically before the restore begins. If the restore fails, Tracearr will
  attempt to roll back to this point.
</Callout>

<Callout type="warning">
  Restoring a large database can be memory intensive. Ensure your container has adequate memory
  allocated before starting a restore. For large databases, we recommend at least **4GB** of memory.
</Callout>
...
### Destination Secrets and the Encryption Key

A backup is a dump of the whole database, so notification destinations, newsletters, their send history and the email suppression list all travel with it.

The configuration of each destination is stored encrypted, under `ENCRYPTION_KEY` when that is set and under a key derived from `JWT_SECRET` otherwise. Restore into an install whose secret differs from the one the backup was written under and those rows cannot be read. Every affected destination is flagged "Re-enter this destination's secret" in the list and is skipped until you do, so an SMTP password or a webhook URL has to be typed in again. Keep whichever secret applies alongside the backup file and none of that happens.
...
<Callout type="info">
  If you set `BACKUP_DIR`, make sure your Docker volume or bind mount targets the same path. For
  example, if you set `BACKUP_DIR=/backups`, your compose mount should be `/path/on/host:/backups`.
</Callout>
