...
A title is dated by when Tracearr first saw it, not by the date the media server reports. Server added dates move: a rescan, a rebuilt library, or a file replaced in place can reset them, and a title you have had for years arrives looking new. The date Tracearr records does not move once the item exists.

Two things follow from that.

Connect a server and its whole library is new to Tracearr, however old the files are. The first newsletter after connecting one lists everything the window reaches, so a 31 day window covers the lot. Send it manually first, or set a short window for the first run, before the schedule takes over.

Rebuild or merge a library on the server and its titles are new again. The server issues new ids, Tracearr records items it has not seen before, and the next newsletter lists them. The library is fine; the newsletter is reporting what changed on Tracearr's side.
...
The Recipients card holds the **Everyone on the chosen servers** switch, a summary of who the next send reaches with **Manage recipients** to exclude people, and **Add people without an account** for typed addresses. All of it is on the [Recipients and unsubscribes](/configuration/email/recipients) page.

## Delivery

### Email destination

The SMTP account this sends through. It does not decide who receives it, and it does not use the destination's **Alert recipients** list. With no email destination saved anywhere, the field offers **Add email destination** in place of the picker. The dialog it opens is titled **Add email server** and only asks for the SMTP server and sender, leaving out **Alert recipients** and **Receive violations**: "Newsletter recipients are set on the newsletter itself. To send alerts through this server too, edit it later under Destinations."
...
The Recipients row answers for the form as it stands, saved or not, and reads "Checking who receives this newsletter." while it works that out.
