...
<Callout type="info">
  A local network session stays local even when its server has a
  [location](/configuration/servers#server-location): **Country** never matches it, and **Active
  Session Distance** and **Travel Speed** leave it out.
</Callout>
...
Sends an alert to the notification destinations you pick. An email destination is one of the choices; see [Email & Newsletters](/configuration/email) for its setup and [Notification destinations](/configuration/destinations) for the rest.
...
| Trigger                     | Fires when                                                             |
| --------------------------- | ---------------------------------------------------------------------- |
| A stream starts             | Playback is confirmed, about thirty seconds after Tracearr sees it     |
| A stream is first seen      | Tracearr first notices a stream, before the thirty-second confirmation |
...
**A stream is first seen** fires the moment Tracearr notices a new stream. Every stream is held for about thirty seconds before it is recorded, which keeps phantoms out of your history, and **A stream starts** fires once that hold clears. First seen doesn't wait. That has a cost: a stream that dies during the hold, someone pressing stop straight away or a player that never really got going, has already fired, on a session your history will never show. An alert a person reads can afford the thirty seconds. A webhook another app acts on usually can't, and that's what this trigger is for.
...
| Trigger                 | Fires when                                                    |
| ----------------------- | ------------------------------------------------------------- |
| A newsletter is sent    | A scheduled or manual newsletter reached everyone on its list |
| A newsletter send fails | A newsletter reached nobody, or only part of its list         |

Both carry `{{newsletter.name}}`, `{{newsletter.outcome}}`, `{{newsletter.recipientCount}}` and `{{newsletter.error}}`.

These two are about the install rather than a person or a session, so they supply no user, no account and no server. That rules them out of a policy automation, and out of any action that needs somebody to act on.
...
**New automation** opens a gallery of automations that are already built. Twenty-two ship with Tracearr, in four groups:

| Group            | What's in it                                                                                                                                                                                                                                            |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Notifications    | Tell a destination something happened: a stream started or stopped, a stream is transcoding, a stream sat paused too long, media was added or upgraded, an account played from a new device, an account's trust score changed, a newsletter send failed |
...
**Stream-start alerts arrive a little later.** They fire once Tracearr has confirmed the session, about thirty seconds after it first sees it. For anything that can't wait that long, the **A stream is first seen** trigger fires at first sight instead.
