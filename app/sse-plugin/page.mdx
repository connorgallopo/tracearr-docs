...
<Callout type="info">
  The plugin streams more than Tracearr currently consumes (scheduled task events, for example). New
  Tracearr features build on this stream, so installing the plugin now means later releases light up
