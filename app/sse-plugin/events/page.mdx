...
<Callout type="info">
New event types get added over time (the [changelog](https://github.com/Tracearr/Media-Server-SSE/releases) lists them per release). Unknown event names are safe to ignore, so a client that only handles the events it knows about keeps working across plugin updates.
</Callout>
